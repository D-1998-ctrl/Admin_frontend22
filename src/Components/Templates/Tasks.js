            ...
            {SubtaskSwitch && (
              <div className='subtask-input'>
                <div style={{ display: 'flex', gap: '30px', alignItems: 'center' }}>
                  ...
                  <RiDeleteBin6Line className='task-bin-icon' />
                  <PiDotsSixVerticalBold className='task-six-dots' />
                </div>
                {subtasks.map(subtask => (

                  <div key={subtask.id} style={{ display: 'flex', gap: '30px', alignItems: 'center' }}>

                    <input type="checkbox" className='subtask-checkbox' placeholder='subtask' />
                    <input type="text" placeholder='Things To do' value={subtask.text} onChange={(e) => handleInputChange(subtask.id, e.target.value)} />
                    <RiDeleteBin6Line onClick={() => handleDeleteSubtask(subtask.id)} className='task-bin-icon' />
                    <PiDotsSixVerticalBold className='task-six-dots' />
                  </div>
