...
const InvoicesTemp = ({ charLimit = 5000 }) => {
  ...
  const totalPages = Math.ceil(invoiceTemplates.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = Math.min(startIndex + itemsPerPage, invoiceTemplates.length);
  const currentTemplates = invoiceTemplates.slice(startIndex, endIndex);
