...
const handleShortcodeClick = (shortcode) => {
  ...
  const fetchDataemail = async () => {
    try {
      const response = await fetch("http://192.168.1.116:8080/workflow/chattemplate");
      const data = await response.json();
      console.log(data);
   
      setUserTemp(data.chatTemplate
      );
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  ...
  const fetchDataemaildetails = async (selecttempId) => {
    try {
      const response = await fetch("http://192.168.1.116:8080/workflow/chattemplate/" + selecttempId);
      const data = await response.json();

         console.log(data.chatTemplate.description
         );
      const contentBlock = htmlToDraft(data.chatTemplate
        .description
      );
      const contentState = ContentState.createFromBlockArray(contentBlock.contentBlocks);
      setEditorState(EditorState.createWithContent(contentState));
      setHtmlContent(data.chatTemplate
        .description);
      setFetchTemplateData(data.chatTemplate);
      dataupdated(data.chatTemplate)
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  };
  ...
  const datasend = () => {
    ...
    fetch("http://192.168.1.116:8080/templatChatEmailsend", requestOptions)
    ...
    <div className='from' style={{ padding: "50px" }}>
      <div className="form__row">
        <div className="form_col form_col_100">
          ...
          <div className="_select_5n3c2_115">
            <label className="_selectLabel_5n3c2_221">Chat Template</label>
            <div className="react-select-container css-b62m3t-container">
